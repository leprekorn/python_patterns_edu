...
def test_allocations_view(make_real_uow_and_messagebus):
    ...
    assert views.allocations(order_id="order1", uow=uow) == {"sku": "sku1", "batchref": "sku1batch"}
    assert views.allocations(order_id="otherorder", uow=uow) == {"sku": "sku1", "batchref": "sku1batch-later"}
